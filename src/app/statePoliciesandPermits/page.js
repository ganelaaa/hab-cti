...
import { getstatePoliciesandPermitsFields } from "@/lib/cms";
...
export default function RegulationsDirectory() {
  ...
  useEffect(() => {
    async function loadDirectoryFields() {
      const data =
        await getstatePoliciesandPermitsFields();
  ...
  const pageDescription =
    ...
    "Select a state to view its State Policies and Permits information.";
  ...
  return (
    <div className="px-4 py-5 tracking-wide sm:px-10 lg:px-20">
      {/* Breadcrumb */}
      <div className="mb-4 flex flex-row items-center gap-2 text-sm text-gray-500">
        <svg
      ...
      >
        ...
        {activeState && (
          <div
            className="absolute z-20 w-max max-w-[300px]"
            style={{
              left:
                activeState.anchorX,
          ...
          >
            <div className="relative rounded-md border border-gray-300 bg-white px-3 py-2 shadow-md">
              {/* State name */}
              <p className="whitespace-nowrap border-b border-gray-200 pb-1.5 text-sm font-bold leading-tight text-primary-darker">
                {activeState.name}
              </p>
