...
//   chemical: "science",
//   biological: "hourglass_empty",
//   physical: "wash",
//   minimum: "assignment_turned_in",
// };

// export default function Strategies({ strategiesList = [] }) {
...
//     // SAFE PARSING: If mainTab is an array, take the first element. Otherwise use it as a string.
//     let mainKey = fields?.mainTab || "chemical";
//     if (Array.isArray(mainKey)) {
//       mainKey = mainKey[0] || "chemical";
//     }
    
//     // Ensure it is definitely a lowercase string just in case
//     mainKey = String(mainKey).toLowerCase();

//     if (!acc[mainKey]) {
//       acc[mainKey] = {
//         label: mainKey === "minimum" 
//           ? "Minimum Risk Pesticides" 
//           : mainKey.charAt(0).toUpperCase() + mainKey.slice(1),
//         icon: MAIN_TAB_ICONS[mainKey] || "science",
//         subTabs: {}
//       };
//     }
...
//       if (availableSubKeys.length > 0 && !availableSubKeys.includes(activeSubTab)) {
...
//         No control technologies configured yet. Check server terminal logs for validation info.
//       </div>
//     );
//   }
...
//       <h2 className="mt-1 text-3xl font-bold text-black">Types of HAB Control Technologies</h2>
...
//                 activeTab === key ? "bg-[#4b710a] text-white" : "text-gray-500 hover:bg-[#bed79c]"
//               }`}
//             >
...
//                 activeSubTab === key ? "bg-primary text-white" : "text-gray-500 hover:bg-[#bed6e8]"
//               }`}
//             >
//               {subTab.label}
//             </button>
...
//               <img src={currentContent.image} alt="" className="w-60 h-60 object-contain shrink-0 mx-auto md:mx-0 mt-2" />
...
//                   <div 
//                     dangerouslySetInnerHTML={{ __html: currentContent.htmlBody }} 
...
export default function Strategies({ strategiesList = [] }) {
  ...
  const tabs = strategiesList.reduce((acc, currentPost) => {
    ...
    const mainKey = String(tabLabel).toLowerCase().replace(/[^a-z0-9]+/g, '-');
        ...
        <div className="bg-[#ecf8dc] flex flex-wrap text-gray-500 font-bold p-4 gap-2">
          {Object.entries(tabs).map(([key, tab]) => (
            <button
              ...
              onClick={() => handleTabClick(key)}
              className={`p-4 rounded transition-all duration-300 cursor-pointer flex items-center justify-center gap-2 flex-1 min-w-[140px] ${
                (activeTab === key || (!tabs[activeTab] && Object.keys(tabs)[0] === key)) 
